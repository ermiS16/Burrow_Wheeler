...
from view.Warning import Warning
from view.Warning import WARNING
...
class Gui(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self._main_frame_widget = QWidget(self)
        self._content = None
        self._warning = None
    ...
    def resetWindow(self):
        childs = self._main_frame_widget.children()
        for child in childs:
    ...
    def updateColor(self):
        ...
        if type == ColorType.label.value:
            if self._control_panel.getDirection() == Direction.forward.value:
                if self._state.getState() == STATE.F_INDEX_SELECT or self._state.getState() == STATE.F_END:
    ...
    def initUI(self):
        ...
        self._control_panel.setParent(self._main_frame_widget)
        ...
        self._content.setParent(self._main_frame_widget)
        self._content.show()


        self.setCentralWidget(self._main_frame_widget)
        self.show()
    ...
    def transform(self):
        ...
        if direction == Direction.forward.value:
            ...
            if(len(self._f_input_text) < 3):
                input_valid = False
                self._warning.showWarning(WARNING.NO_INPUT)
        ...
        if direction == Direction.backwards.value:
            ...
            if len(encode) == 0:
                ...
                input_valid = False
                self._warning.showWarning(WARNING.NO_INDEX)

            if input_valid:
                if int(index) > len(encode):
                    self._warning.showWarning(WARNING.INVALID_INDEX)
                    index_valid = False

            if input_valid and index_valid:
        ...
        if direction == Direction.choose.value:
            self._warning.showWarning(WARNING.NO_DIRECTION)
